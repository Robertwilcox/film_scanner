'''
Film Negative Extraction and Adjustment Tool

This script processes images of film negatives, automatically 
detects and crops individual frames, allows for manual adjustment 
of bounding boxes, inverts the negatives to positives, and provides 
...
from bbox_detection import (
    auto_detect_bboxes_with_perforations, 
    detect_initial_boxes, 
    filter_boxes_with_perforation_data
)
...
def mouse_callback(event, x, y, flags, param):
    """
    Handle mouse events for interacting with bounding boxes, 
    including resizing and moving.

    Args:
        event (int): Type of mouse event (e.g., click, move).
        x (int): x-coordinate of the mouse event.
        y (int): y-coordinate of the mouse event.
        flags (int): Any relevant flags associated with the event.
        param (any): User-defined parameter (scale factor in this case).
    """
    ...
    if event == cv2.EVENT_LBUTTONDOWN:  # Left-click
        ...
        for idx, (bx, by, bw, bh) in enumerate(bboxes):
            ...
            for i, (ax, ay) in enumerate(anchors):
                if abs(scaled_x - ax) <= anchor_radius and \
                   abs(scaled_y - ay) <= anchor_radius:
                    selected_idx = idx
                    anchor_being_dragged = (i, idx)  # Anchor and bbox index
                    return
    ...
    elif selected_idx != -1 and flags == cv2.EVENT_FLAG_LBUTTON:  # Move box
    ...
    elif event == cv2.EVENT_LBUTTONDBLCLK:  # Double-click to remove a box
        for idx, (bx, by, bw, bh) in enumerate(bboxes):
            if bx <= scaled_x <= bx + bw and by <= scaled_y <= by + bh:
                bboxes.pop(idx)
...
def adjust_colors(image, output_path):
    """
    Display an image with color adjustment sliders and save the 
    adjusted version.
    ...
    def on_trackbar_change(val):
        ...
        adjusted_image[:, :, 2] = np.clip(
            adjusted_image[:, :, 2] + red_adj, 0, 255
        )  # Red
        adjusted_image[:, :, 1] = np.clip(
            adjusted_image[:, :, 1] + green_adj, 0, 255
        )  # Green
        adjusted_image[:, :, 0] = np.clip(
            adjusted_image[:, :, 0] + blue_adj, 0, 255
        )  # Blue
    ...
    print("Adjust colors using sliders. Press 's' to save or 'q' to quit.")
...
def process_image(image_path):
    """
    Process a single image: detect and refine bounding boxes, 
    allow user adjustment, crop, invert, and adjust colors.
    ...
    bboxes = filter_boxes_with_perforation_data(
        initial_bboxes, perf_stats, image.shape
    )
    ...
    # Set up the main window for bounding box adjustment
    ...
    try:
        ...
        # Crop, invert, and adjust colors for each bounding box
        for idx, (x, y, w, h) in enumerate(bboxes):
            ...
            adjusted_path = os.path.join(
                OUTPUT_DIR, 
                f"{os.path.splitext(os.path.basename(image_path))[0]}"
                f"_negative_{idx + 1}_adjusted.jpg"
            )
