'''
Film Perforation Detection and Analysis

This script detects perforations in film negative images, 
validates them based on brightness and size, filters 
overlapping perforations, and calculates average 
...
DEBUG_LOG_PATH = os.path.join(OUTPUT_DIR, "debug_log.txt")  # Debug log file
SUMMARY_LOG_PATH = os.path.join(OUTPUT_DIR, "summary_log.txt")  # Summary log
...
PARAMS = {
    "adaptive_thresh_block_size": 17,  # Block size for adaptive thresholding
    ...
    "rect_size_range": (20, 600),     # Min/max width/height for rectangles
    "shrink_factor": 0.3,            # Shrinkage for brightness validation
    "max_std_dev": 5,                # Max standard deviation inside subset box
    "brightness_threshold_factor": 0.8,  # Brightness factor for validation
}
...
def validate_perforation_brightness(image, rectangles, params):
    """
    Validates detected rectangles as perforations based on 
    brightness using subset boxes.

    Args:
        image (numpy.ndarray): The grayscale image.
        rectangles (list): List of rectangle points.
        params (dict): Dictionary of parameters.

    Returns:
        list: List of validated rectangles.
    """
    max_brightness = np.max(image) 
    brightness_threshold = max_brightness * params["brightness_threshold_factor"]
    write_debug_log(
        f"STEP: Max Brightness: {max_brightness:.2f}, "
        f"Threshold: {brightness_threshold:.2f}"
    )
    ...
    for box in rectangles:
        ...
        if subset_brightness < brightness_threshold:
            ...
            write_debug_log(
                f"Rejected: Brightness={subset_brightness:.2f} "
                f"< Threshold={brightness_threshold:.2f}"
            )
        elif subset_std_dev > params["max_std_dev"]:
            rejected_by_std_dev += 1
            write_debug_log(
                f"Rejected: StdDev={subset_std_dev:.2f} "
                f"> MaxStdDev={params['max_std_dev']:.2f}"
            )
        else:
    ...
    write_summary_log(
        ...
        f" - Total: {len(rectangles)}\n"
        f" - Validated: {len(validated_rectangles)}\n"
        f" - Rejected (brightness): {rejected_by_brightness}\n"
        f" - Rejected (std dev): {rejected_by_std_dev}"
    )
...
def detect_perforations(image, params, debug=False):
    """
    Detect perforations in the film image based on contours and 
    aspect ratio.
    ...
    Returns:
        list: List of detected rectangles (perforations).
    """
    ...
    write_debug_log(f"STEP: Found {len(contours)} contours.")
    ...
    for contour in contours:
        ...
        if contour_area < params["min_contour_area"]:
            write_debug_log(f"Rejected: Area={contour_area:.2f} (too small)")
            if debug:
                cv2.drawContours(rejected_contours_image, [contour], -1, (0, 0, 255), 1)
            continue
        ...
        write_debug_log(
            f"Contour: Area={contour_area:.2f}, Width={width:.2f}, "
            f"Height={height:.2f}, Aspect Ratio={aspect_ratio:.2f}"
        )
        ...
        if not (
            ...
            write_debug_log("Rejected: Failed aspect ratio or size filter")
            if debug:
                cv2.drawContours(rejected_contours_image, [box], -1, (255, 0, 0), 1)
            continue
    ...
    if debug:
        ...
        for box in rectangles:
            cv2.drawContours(candidate_debug, [box], -1, (0, 255, 0), 1)
    ...
    write_summary_log(
        ...
        f" - Rejected (area/aspect): {len(contours) - len(rectangles)}"
    )
...
def calculate_average_dimensions(rectangles, std_threshold=2.0):
    """
    Calculates the average dimensions (width, height, aspect ratio, 
    area) of perforations, accounting for rotation and skew, and 
    filtering out outliers.

    Args:
        rectangles (list): List of rectangle points.
        std_threshold (float): Threshold for filtering outliers.

    Returns:
        dict: A dictionary with average dimensions and std deviations.
    """
    dimensions = []
    for box in rectangles:
        # Calculate the rotated bounding box
        rot_rect = cv2.minAreaRect(box)
        width, height = rot_rect[1]  # Get width and height
...
if __name__ == "__main__":
    for filename in os.listdir(INPUT_DIR):
        if filename.lower().endswith(".jpg"):
            ...
            non_overlapping_perforations = filter_overlapping_rectangles(
                validated_perforations, iou_threshold=0.5
            )

            # Calculate average dimensions and filter outliers
            avg_dims = calculate_average_dimensions(
                non_overlapping_perforations, std_threshold=2.0
            )
